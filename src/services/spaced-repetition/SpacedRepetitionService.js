...
export class SpacedRepetitionService {
  ...
  /**
   * Calculate next review date using the SM-2 algorithm with adaptive modifications
   ...
   */
  calculateNextReview(item, rating, params = null) {
   ...
   */
  async submitReview(sessionId, rating) {
    const session = this.activeReviewSessions.get(sessionId);
    ...
    // Calculate next review date
    const updatedItem = this.calculateNextReview(currentItem, rating);
